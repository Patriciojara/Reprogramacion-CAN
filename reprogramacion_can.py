...
# GPIO de alimentación del STM32
# Pin físico 11 = BCM GPIO17
POWER_GPIO = 17
...
def valid_canfd_len(length: int) -> int:
    """
    CAN-FD usa DLC discretos:
    0-8, 12, 16, 20, 24, 32, 48, 64.
    Esta función redondea al tamaño válido superior.
    """
...
def wait_resp(bus: can.BusABC, expected_cmd: int, expected_seq=None, timeout=1.0):
    ...
    while time.time() - t0 < timeout:
        ...
        if status == RESP_NACK:
            raise RuntimeError(
                f"NACK recibido: cmd=0x{cmd:02X}, err=0x{err:02X}, seq={seq}"
            )

        raise RuntimeError(
            f"Respuesta desconocida: status=0x{status:02X}, cmd=0x{cmd:02X}, err=0x{err:02X}"
        )
...
def begin_update(bus, fw_size: int, fw_crc: int):
    ...
    # Este comando borra Flash; dale más tiempo.
...
def send_data_block(bus, seq: int, offset: int, chunk32: bytes):
    ...
    # Payload real = 39 bytes; se rellena a 48 para CAN-FD.
    send_fd(bus, bytes(payload))
...
def end_update(bus):
    ...
    # Calcula CRC en el STM32; puede tardar.
...
def power_cycle_stm32(active_high=True, off_time=0.5, boot_delay=0.05):
    """
    active_high=True:
      GPIO17 = 1 enciende STM32
      GPIO17 = 0 apaga STM32

    Si tu hardware es al revés, usa --active-low.
    """
    ...
    # Pequeña espera para que suba la fuente y arranque el bootloader.
...
def find_bootloader(bus, tries=40, delay=0.05):
    """
    Busca el bootloader enviando PING repetidamente.
    40 intentos x 0.05 s = 2 s aprox.
    """
...
def main():
    ...
    parser.add_argument(
        "firmware",
        help="Archivo .bin de la aplicación principal"
    )

    parser.add_argument(
        "--iface",
        default=CAN_IFACE,
        help="Interfaz CAN. Por defecto: can0"
    )

    parser.add_argument(
        "--no-config-can",
        action="store_true",
        help="No configurar can0 automáticamente"
    )

    parser.add_argument(
        "--no-power-cycle",
        action="store_true",
        help="No apagar/encender el STM32 con GPIO17"
    )

    parser.add_argument(
        "--active-low",
        action="store_true",
        help="Usar GPIO17 activo bajo. GPIO17=0 enciende, GPIO17=1 apaga"
    )

    parser.add_argument(
        "--no-run",
        action="store_true",
        help="No saltar a la aplicación al terminar"
    )
    ...
    fw, fw_padded, fw_size, fw_crc = load_firmware(args.firmware)
    ...
    # Mantener referencia viva hasta el final.
    _ = power_handle
